import {
  darkTheme
} from "./chunk-H522HX64.js";
import {
  midnightTheme
} from "./chunk-V2QD2P42.js";
import {
  ConnectButton,
  DesktopOptions,
  MobileOptions,
  ...
  useChainId,
  useTransactionStore
} from "./chunk-AQQUGKJB.js";
import {
  lightTheme
} from "./chunk-5YTEF3J5.js";
import "./chunk-TCHUATDC.js";
...
var fallback = () => {
  return function(chain) {
    ...
    return {
      chain,
      provider: () => new providers2.StaticJsonRpcProvider(chain.rpcUrls.default, chain.id)
    };
  };
};

// src/chains/apiProviders/infura.ts
import { providers as providers3 } from "ethers";
...
var infura = (infuraId = defaultInfuraId) => {
  return function(chain) {
    ...
    return {
      ...
      provider: () => new providers3.InfuraProvider(chain.id, infuraId),
      webSocketProvider: () => new providers3.InfuraWebSocketProvider(chain.id, infuraId)
    };
  };
};

// src/chains/apiProviders/jsonRpc.ts
import { providers as providers4 } from "ethers";
var jsonRpc = (getRpcUrls) => {
  return function(chain) {
    ...
    return {
      ...
      provider: () => new providers4.StaticJsonRpcProvider(rpcUrl, chain.id),
      ...webSocketRpcUrl && {
        webSocketProvider: () => new providers4.WebSocketProvider(webSocketRpcUrl, chain.id)
      }
    };
  };
};
...
var configureChains = (defaultChains, apiProviders) => {
  ...
  for (const chain of defaultChains) {
    ...
    for (const apiProvider2 of apiProviders) {
      ...
      providers5[chain.id] = apiConfig.provider;
...
`,
  ...
  return {
    chains,
    provider: ({ chainId }) => {
      return providers5[chainId && chains.some((x) => x.id === chainId) ? chainId : defaultChains[0].id]();
    },
...
var ledger = ({ chains }) => ({
  ...
  createConnector: () => {
    const rpc = rpcUrlsForChains(chains);
    const connector = new WalletConnectConnector3({
      chains,
      options: {
        qrcode: false,
        rpc
...
var metaMask = ({
  ...
  return {
    ...
    createConnector: () => {
      const rpc = rpcUrlsForChains(chains);
      const connector = shouldUseWalletConnect ? new WalletConnectConnector4({
        chains,
        options: {
          qrcode: false,
          rpc
...
var walletConnect = ({ chains }) => ({
  ...
  createConnector: () => {
    const ios = isIOS();
    const rpc = rpcUrlsForChains(chains);
    const connector = new WalletConnectConnector7({
      chains,
      options: {
        qrcode: ios,
        rpc
...
import { WalletConnectConnector as WalletConnectConnector8 } from "wagmi/connectors/walletConnect";
    ...
    walletList.forEach(({ groupName, wallets }) => {
      wallets.forEach(({ createConnector, ...walletMeta }) => {
        ...
        if (walletMeta.id === "walletConnect" && connectionMethods.qrCode) {
          ...
          walletConnectModalConnector = new WalletConnectConnector8({
            chains,
            options: {
              ...options,
              qrcode: true
